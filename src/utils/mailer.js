...
export const enviarCorreoBienvenida = async (emailDestino, nombreFan) => {
    const mailOptions = {
        from: `"Savage Group" <${process.env.EMAIL_USER}>`,
        to: emailDestino,
    ...
    try {
        ...
        console.log(`📧 Correo Fan enviado a: ${emailDestino}`);
    } catch (error) {
...
export const enviarDossierComercial = async (emailDestino, nombreContacto, empresa) => {
    const mailOptions = {
        from: `"Savage Group B2B" <${process.env.EMAIL_USER}>`,
        to: emailDestino,
    ...
    try {
        await transporter.sendMail(mailOptions);
        console.log(`💼 Correo B2B enviado a: ${empresa}`);
    } catch (error) {
